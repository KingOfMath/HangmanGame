...
class HangmanGame:
    def __init__(self, word, failed_guesses_limit):
        ...
        self.undoChance = False
        self.lastLetterRevealed = False
    ...
    def undo(self):
        ...
        if not self.guesses:
            return GuessResult.FAIL_EMPTY_INPUT

        if self.undoChance:
            return GuessResult.FAIL_ALREADY_UNDO
        self.undoChance = True

        # reset
        if self.lastLetterRevealed:
            lastLetter = self.guesses[-1]
            self.guesses.remove(lastLetter)

            for i in range(len(self.word)):
                curr_letter = self.word[i]

                if lastLetter == curr_letter:
    ...
    def guess(self, input_letter):
        ...
        self.lastLetterRevealed = is_letter_in_word
