...
@api.route('/api/hangman/<int:game_id>/undo', methods=['POST'])
def undo_hangman_guess(game_id):
    game = game_manager.get_game(game_id)
    ...
    if result == hangman.GuessResult.FAIL_ALREADY_GAME_OVER:
        return jsonify({'error': 'Game already over'}), 500
    elif result == hangman.GuessResult.FAIL_EMPTY_INPUT:
        return jsonify({'error': 'Empty input'}), 500
    elif result == hangman.GuessResult.FAIL_ALREADY_UNDO:
        return jsonify({'error': 'Undo chance already used'}), 500
